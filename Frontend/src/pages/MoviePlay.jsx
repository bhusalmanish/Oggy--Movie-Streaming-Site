import React, { useState } from 'react'
import Rating from '@mui/material/Rating';
import Card from '@mui/material/Card';
import CardMedia from '@mui/material/CardMedia';
import { Box, Button, Container, Grid } from '@mui/material';
import ReactPlayer from 'react-player/lazy'
import Stack from '@mui/material/Stack';
import { useNavigate ,useLocation} from 'react-router-dom';

import Movies  from '../Data/Movies.json'
import MoviesImageCard from '../components/utilsComponets/MoviesImageCard';

import Avatar from '@mui/material/Avatar';
import ThumbUpOffAltIcon from '@mui/icons-material/ThumbUpOffAlt';
import ThumbDownOffAltIcon from '@mui/icons-material/ThumbDownOffAlt';
import ThumbUpIcon from '@mui/icons-material/ThumbUp';
import ThumbDownIcon from '@mui/icons-material/ThumbDown';


export default function MoviePlay() {
  return (
    <div>
      <Grid sx={{padding:"40px"}} container>   
    <Grid  xs={8} item>
    <WatchMovie/>
     </Grid>
    <Grid  xs={4} item sx={{backgroundColor:"#F6F1E9", padding:"8px"}} > 
    <SuggestMovie/>
  
</Grid>
      </Grid>
 
    </div>
  )
}


const WatchMovie = ()=>{

  const  navigate = useNavigate();
  const  location  = useLocation()
  const [isShown, setIsShown] = useState(false);
  const[url, SetUrl] = useState("");
  const Movie = location.state.M;

   const HandelTailer= (M)=>{ 
         SetUrl(M);
         setIsShown( current => !current)}
  return(<>
   <Container> 
      <Button    variant="contained" onClick={()=>{ navigate("/")}}> X </Button>
      <Card sx={{ maxWidth: 900, padding: 2 }}>
        <Grid container >
          <Grid xs={4} item sx={{ backgroundColor: "#F6F1E9" }} > <CardMedia sx={{ height: "500px", width: "300px" }}
            component="img"
            image={Movie.Movie_image}
            alt="green iguana"
          />
          </Grid >
          <Grid xs={8} item sx={{backgroundColor :"#F6F1E9",padding: 2 }} >
            <center>
              <h2 >{Movie.name}</h2>
              <p>Realease On: {Movie.Realease_date}</p>
              <p> Categories: Drama : Action: Si-Fi</p>
              <p>  lenght:2.2 Hrs :  4.2M views  </p>
              <p> Rating: <Rating defaultValue={Movie.Rating}/>  </p>
              <p> Director : {Movie.Director} </p>
              <p> Actors:  {Movie.Actor +  '.'} </p> 
              </center>
              <p> Description: {Movie.Movie_description}</p>
          </Grid>
        </Grid>
        <Stack spacing={2} direction="row" sx={ {  mt: "20px",justifyContent: 'space-evenly'}}>
          <Button   onClick={()=>{HandelTailer(Movie.Movie_tailers)}} variant="contained"> Watch Tailer</Button>
          <Button   onClick={()=>{HandelTailer(Movie.video_link)}} variant="outlined"> Play Now!!</Button>
        </Stack>
      </Card>
           {isShown === true? <ReactPlayer  width="880px"  height={"500px"} pip={true} playing={true} controls={true} url={url}/> : ""}
           <MoiveComments/>

           </Container>
  
  </>)
}


const  SuggestMovie = ()=>{
  const Movie = Movies;
  return(
    <> 

    <Grid container spacing={1} >   
  {  Movie.slice(0,6).map((M, index)=>{

    return(
      <Grid   item >   
      <div key={index}>  <MoviesImageCard M ={M}/></div>
      </Grid>)
    })} 
        </Grid>
    </>
  )
}



const MoiveComments =()=>{
  return(<>
    <Card  sx={{ backgroundColor:"#ffe7a2", marginTop:"20px", padding:"10px"}}>
      <h5>Comments:</h5>
      <Box sx={{margin:"10px", backgroundColor:"#f5d25b96", padding:"10px", borderRadius:"10px"}} >
      <Stack direction="row" spacing={2}  >
      <Avatar   height="40px" alt="Remy Sharp" src="data:image/png;base64," />
      <input     type='text' style={{ width:"100%", padding:"10px", borderRadius:"5px" }}  placeholder='Movie Reviews'/>
    </Stack>
      </Box>
      {/* comments  */}
       

      <Box sx={{margin:"10px", marginBottom:"0px" , backgroundColor:"#f5d25b96", padding:"10px", borderRadius:"10px"}}>
        <OtherComments    Name={"Boli Zipper"}   Time={"2hrs."}Icon={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTslKnx10zLRjVYDZ5SmOsQKLnG-OdUx9VCeg&usqp=CAU"} Comment={"wow best Movies Ever"}/>
      <OtherComments   Name={"John White"  } Time={"3hrs."}Icon={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSAaeVfXxyG1sNBohvr-x5NOCzM9lvcF_pTzA&usqp=CAU"}  Comment={" wow This movies made my day!!"}/>

      <OtherComments    Name={"Boli Zipper"}   Time={"2hrs."}Icon={"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTslKnx10zLRjVYDZ5SmOsQKLnG-OdUx9VCeg&usqp=CAU"} Comment={"Thrile movie  32:34 is best  moments"}/>

      </Box>
      
    </Card>
  </>)
}




const OtherComments = (props)=>{
          let [Icontype, SetIconType] =  useState(false);

   const IconClick= (e)=>{
     SetIconType(e)
   }
  return(<>
  
  <Stack direction="row" spacing={3}  sx={{  marginBottom:"0px",'&:hover':{ backgroundColor: "#ffe7a2" ,borderRadius:"10px", padding:"2px"}}}>
      <Avatar alt="Remy Sharp" src={props.Icon}/>
        <Stack sx={{marginTop:"1px", spacing:"0.1", width:"100%"}} >
          <div>   
              <h6  style={{float:"left", marginRight:"10px", marginBottom:"0px"}}> {props.Name}</h6>  <span> {props.Time}</span>  
              </div>
             <p   style={{marginBottom:"0px"}}>  {props.Comment}</p>
              <Stack direction="row" >
              {Icontype=== false ?<ThumbUpOffAltIcon  onClick={()=>{  IconClick(true) }}  />: <ThumbUpIcon  onClick={()=>{ IconClick(false)}}/>}
              2:{Icontype=== false ?  <ThumbDownOffAltIcon onClick={()=>{IconClick(true)} }/>: <ThumbDownIcon onClick={()=>{ IconClick(false)}}/>} :34
  {/* 2: <ThumbUpOffAltIcon  onClick={()=>{ IconClick(true)}}/>
<ThumbDownOffAltIcon/>:34 */}
              </Stack>
          </Stack>  
    </Stack>
    <hr />
 
  </>)
}